...
class _HomeState extends State<Home> {
  ...
  /// Fetches user's current active goal from Firestore
  ...
  /// Calculates progress towards current goal based on transactions
  void _calculateGoalProgress() {
    ...
    // Show achievement dialog if goal is met
    if (newProgress >= 1.0) {
      _showGoalAchievedDialog();
    }
  }

  /// Shows congratulatory dialog when goal is achieved
  ...
  /// Shows dialog to add or edit a goal
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: Text(_currentGoalId != null ? 'Update Goal' : 'Set Goal'),
          ),
        ],
      ),
    );
  ...
  /// Formats currency amounts with Tanzanian Shilling symbol
  ...
  /// Calculates remaining days until goal deadline
                  ...
                  child: ListView(
                                ...
                                Row(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      _getRemainingDays(),
                                      style: TextStyle(
                                        color: _goalDeadline?.isBefore(DateTime.now()) ?? false
                                            ? Colors.red[300]
                                            : Colors.white70,
                                        fontSize: 11,
                                      ),
                                ...
                                children: [
                                  Expanded(
                                    child: ElevatedButton.icon(
                                      onPressed: () => _showAddNewGoalDialog(),
                                      icon: Icon(
                                        _goalTitle.isEmpty ? Icons.add : Icons.edit,
                                        size: 16,
                                      ),
                                      label: Text(
                                        _goalTitle.isEmpty ? 'Add Goal' : 'Edit Goal',
                    ...
                    ListView.builder(
                      ...
                      itemBuilder: (context, index) => _buildTransactionItem(_transactions[index]),
                        ...
                        subtitle: Text(DateFormat.yMMMd().format(tx.date)),
                        trailing: Text(
                          '${tx.type == my_model.TransactionType.income ? '+' : '-'} ${_formatCurrency(
